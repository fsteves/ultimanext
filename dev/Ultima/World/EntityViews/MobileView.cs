...
namespace UltimaXNA.Ultima.World.EntityViews
{
    ...
    public class MobileView : AEntityView
    {
        ...
        private void InternalSetupLayers()
        {
            ...
            int NotorietyHue = ((Mobile)Entity).NotorietyHue;
            ...
            if (Body.IsHumanoid) 
            {
                ...
                {
                    int RenderHue = Entity.Hue;

                    if (Entity.IsHidden)
                        RenderHue = 0x3E9;

                    if (Entity.Highlight)
                        RenderHue = NotorietyHue;
                    ...
                    {
                        AddLayer(Body, RenderHue);//Entity.Hue);
                    }
                    ...
                    else if (Equipment[drawLayers[i]] != null)
                    {
                        ...
                        if (drawLayers[i] == (int)EquipLayer.Mount)
                        {
                            ...
                            {
                                int MountHue = Equipment[drawLayers[i]].Hue;

                                if (Entity.IsHidden)
                                    MountHue = 0x3E9;
                                if (Entity.Highlight)
                                    MountHue = NotorietyHue;

                                AddLayer(body, MountHue, true);
                            }
                        }
                        else
                        {
                            if (Equipment[drawLayers[i]].ItemData.AnimID != 0)
                            {
                                ...
                                int EquipmentHue = Equipment[drawLayers[i]].Hue;

                                if (Entity.Highlight)
                                    EquipmentHue = NotorietyHue;

                                AddLayer(Equipment[drawLayers[i]].ItemData.AnimID, EquipmentHue);
                            }
                        }
                    }
                }
            ...
            else
            {
                ...
                if (Entity.Highlight)
                    RenderHue = NotorietyHue;
                //Non humanoid mobiles
                AddLayer(Body, RenderHue);
            }
        }
