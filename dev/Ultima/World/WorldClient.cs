...
namespace UltimaXNA.Ultima.World
{
    class WorldClient : IDisposable
    {
        ...
        public void Initialize()
        {
            ...
            Register<MobileAnimationPacketSA>(0xE2, "Character Animation SA", 10, new TypedPacketReceiveHandler(ReceiveMobileAnimationSA));
            Register<ObjectInfoPacketSA>(0xF3, "World Item SA", -1, new TypedPacketReceiveHandler(ReceiveWorldItemSA));
        ...
        private void ReceiveWorldItemSA(IRecvPacket packet)
        {
            ObjectInfoPacketSA p = (ObjectInfoPacketSA)packet;
        ...
        private void ReceiveMobileAnimationSA(IRecvPacket packet)
        {
            MobileAnimationPacketSA p = (MobileAnimationPacketSA)packet;
            Mobile mobile = WorldModel.Entities.GetObject<Mobile>(p.Serial, false);
            if (mobile == null)
                return;

            //mobile.Animate(p.Action, p.FrameCount, p.RepeatCount, p.Reverse, p.Repeat, p.Delay);
            mobile.Animate(p.AnimationType, p.Action, p.Delay);
        }
