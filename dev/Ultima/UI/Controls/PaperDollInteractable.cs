    ...
    class PaperDollInteractable : Gump
    {
        ...
        private void OnEntityUpdated()
        {
            ...
            if (true)
            {
                ...
                int bodyID = ((Mobile)m_sourceEntity).Body.BodyID;
                int gumpBodyID = 12 + (m_isElf ? 2 : 0) + (m_isFemale ? 1 : 0);

                Tracer.Debug(bodyID.ToString());

                GumpPic paperdoll = (GumpPic)AddControl(new GumpPic(this, 0, 0, gumpBodyID, (bodyID >= 987 ? 33780: ((Mobile)m_sourceEntity).Hue ))); //Account for GM Bodies
                paperdoll.HandlesMouseInput = true;
                paperdoll.IsPaperdoll = true;

                switch (bodyID)
                {
                    default: //Default Male/Female/Elf
                        break;
                    case 987:  //GM Body
                        gumpBodyID = 60987;                        
                        GumpPic gmBody = (GumpPic)AddControl(new GumpPic(this, 0, 0, gumpBodyID, ((Mobile)m_sourceEntity).Hue));
                        gmBody.HandlesMouseInput = true;
                        gmBody.IsPaperdoll = true;
                        break;
                    case 990: //Lord Brittish
                        gumpBodyID = 60990;
                        GumpPic lbBody = (GumpPic)AddControl(new GumpPic(this, 0, 0, gumpBodyID, ((Mobile)m_sourceEntity).Hue));
                        break;
                    case 991: //Lord Blackthorn
                        gumpBodyID = 60991;
                        GumpPic btBody = (GumpPic)AddControl(new GumpPic(this, 0, 0, gumpBodyID, ((Mobile)m_sourceEntity).Hue));
                        break;
