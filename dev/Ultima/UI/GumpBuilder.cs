    ...
    static class GumpBuilder
    {
        public static void BuildGump(Gump gump, string[] gumpPieces, string[] gumpLines)
        {
            ...
            for (int i = 0; i < gumpPieces.Length; i++)
            {
                ...
                //TODO: Remove Debug
                //Tracer.Warn(string.Format("GUMP: Params:{0} - Pieces:{1}", gumpParams[0], gumpPieces[i]));
                ...
                switch (gumpParams[0].ToLower())
                {
                    ...
                    case "tooltip":
                        ...
                        //Return the cliloc from gumpPieces then GetString(cliloc) returns the tooltip
                        string cliloctext = provider.GetString(int.Parse(gumpPieces[i].Remove(0, gumpPieces[i].IndexOf(' ') + 1)));

                        if (gump.LastControl != null)
                            gump.LastControl.SetTooltip(cliloctext);
                        else
